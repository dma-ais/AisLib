...
abstract class ExpressionFilterParserBase {
      ...
      protected static <T> Predicate<T> createFilterPredicateForTimeComparison(Class<AisPacketFilters> filterFactoryClass, ExpressionFilterParser.MessageTimeContext ctx) {
                 Predicate<T> filter = null;
                 ...
                 try {
                     ...
                     if ("m.year".equalsIgnoreCase(field)) {
                         ...
                         int month = ctx.INT() != null ? mapIntToCalendarMonth(ctx.INT().getText()) : mapStringToCalendarMonth(ctx.string().getText());
                         ...
                         int dow = ctx.INT() != null ? mapIntToCalendarDayOfWeek(ctx.INT().getText()) : mapStringToCalendarDayOfWeek(ctx.string().getText());
    ...
    private static int mapIntToCalendarDayOfWeek(String text) {
        return Integer.parseInt(text) + 1 % 8; // 1 = monday
    }
    ...
    private static int mapIntToCalendarMonth(String text) {
